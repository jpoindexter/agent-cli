...
assert(connectionSettings.includes("connectionEnvironmentInputs") && read("app/src/appTerminalSurfaceRuntime.ts").includes("environmentForRoot: (root) => connectionEnvironmentInputs(input.connectionSettings.current, root)") && read("app/src/terminalPaneCommands.ts").includes("environment: options.environmentForRoot(root)") && composerSubmission.includes("environment: connectionEnvironmentInputs(context.settings"), "All new chat and terminal runs must carry project environment references without renderer-side secret values");
...
assert(read("app/src/settingsActionsHost.ts").includes('invoke("delete_connection_secret", { key })'), "Full local reset must remove known Keychain connection secrets before clearing metadata");
