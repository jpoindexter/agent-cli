...
- Use VS Code/Codex-like application structure for the shell: compact top status/action bar, left activity rail, switchable side drawers, draggable workbench trays, and bottom status strip. Adapt the pattern to Keelhouse state and actions; do not copy fake window controls, fake browser navigation, or controls that imply unavailable behavior.
