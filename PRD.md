...
- Make the selected agent pane, composer, and visible activity timeline the primary workbench surface. Default to a readable terminal-backed Run view; expose raw terminal as an explicit switchable view for exact TUI interaction. The code editor and browser preview are first-class tools, but they behave like movable, resizable trays around the agent cockpit rather than the default main screen. Trays open, close, and dock freely, but first open defaults to the demo layout: threads drawer, centered run and composer, tabbed right dock, and bottom tray strip.
