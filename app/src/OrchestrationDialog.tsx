...
const OrchestrationChildEditor = ({ child, index, removable, update, remove }: ChildEditorProps) => (
  <fieldset className="orchestration-child">
    ...
    <label className="orchestration-field"><span>Provider</span><select value={child.provider} onChange={(event) => update(child.id, { provider: event.currentTarget.value as ChatProvider })}><option value="codex">Codex</option><option value="claude">Claude</option><option value="opencode">OpenCode</option></select></label>
