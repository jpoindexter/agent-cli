...
- ~~**AGENT-PROFILES:** Claude default, plus Codex/shell profiles without hardcoding one CLI forever.~~ **DONE 2026-07-09** — terminal header now exposes Claude/Codex/Shell profiles, shows command/mode details, persists the selected profile in `workspace.json`, restarts the pane in the current workspace on profile change, and keeps missing-command failures visible without overwriting the saved profile. Verified with `npm run build`, `npm test` (43 tests, including launch profile helpers), `cargo test` (20 tests, including missing-command preflight), `npm run qa:editor`, selected/narrow visual inspection, and `git diff --check`.
...
- ~~**CONTEXT-MENUS:** right-click/Control-click menus across rail, files, editor, terminal, browser, git/diff, and agent panes.~~ **DONE 2026-07-09** — current v0.5 surfaces now expose shared right-click/Control-click menus for the workspace root, recent project rows, file explorer rows, editor tabs, editor text surface, terminal pane, and composer/agent controls. Menus show shortcut labels where active, disabled states, and danger states; destructive file/tab actions reuse the existing confirmations and dirty-buffer guards. `docs/context-menus.md` documents implemented coverage and defers browser preview plus diff/git menus until those surfaces exist. Verified with `npm run build`, `npm test` (53 tests), `npm run qa:editor`, visual inspection of `docs/qa/editor-parity/context-menu.png`, and `git diff --check`.
...
- ~~**ICON-SYSTEM:** polished Codex-quality iconography, labels, tooltips, and status badges.~~ **DONE 2026-07-09** — current v0.5 chrome now uses `lucide-react` through `app/src/icons.tsx` as one SVG icon contract. File-tree object icons, rail actions, editor actions, terminal status pills, composer controls, context menu rows, and QA fixtures use `currentColor` icons on a 16px dense grid. Non-universal controls keep labels, tooltips, or accessible names. Pane status and future activity status mappings cover thinking/running/waiting/error/exited/complete. Browser, git/diff, source-host, and multi-pane icons remain deferred until those real surfaces land. Verified with `npm run build`, `npm test`, `cargo test`, `cargo fmt --check`, `npm run qa:editor`, screenshot inspection, and `git diff --check`.
