...
`folder` is the last workspace to reopen. `launchProfile` is a compatibility mirror of the structured-chat provider and normalizes to Codex until another structured adapter ships. `terminalLaunchProfile` is used only when the user explicitly opens a raw terminal; it defaults to Shell (`/bin/zsh -l`). Built-in profile ids are `codex`, `gemini`, `claude`, and `shell`. `customLaunchProfiles` stores up to 20 user-managed raw-terminal label/command records with `custom:` ids; these profiles never imply structured chat support. Opening a terminal never starts an agent unless the user explicitly changes that terminal's profile.
...
`scopedSettings` is the authoritative Global -> Project -> Chat override store for chat provider, permission mode, and browser preview URL. Missing Project/Chat keys inherit from their parent. Existing raw-terminal values accidentally migrated into `agentProfileId` normalize back to Codex; terminal selection remains separate in `terminalLaunchProfile`. On first launch after this schema lands, existing browser and compatible composer records seed it; the legacy keys remain readable during the transition.
