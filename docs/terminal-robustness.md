...
**What shipped:** a hidden `<textarea class="terminal-ime-input">` (xterm.js's established pattern) now holds real DOM focus for terminal panes instead of the canvas. It's positioned over the cursor cell every paint tick (`imeCaretStyle` in `terminalIme.ts`, scaled by cell width/height) so the OS candidate window anchors near the caret. `onCompositionEnd` sends the final composed string through the existing `paste` invoke path and clears the textarea. Ordinary ASCII and navigation keydowns still use the existing `send_key` path; only IME/dead-key events are yielded to WebKit. `FONT_FAMILY` gained an explicit CJK fallback chain (`"PingFang SC", "Hiragino Sans", "Apple SD Gothic Neo"`) ahead of generic `monospace`, since JetBrains Mono has no CJK glyphs.

**2026-07-14 correction:** the global key handler could still call `preventDefault()` on the first `Dead` or IME process key before WebKit set `isComposing`, preventing composition from starting. `shouldDeferTerminalKeyToIme` now yields `Dead`, `Process`, legacy key-code 229, and active composition events to the focused textarea. Unit coverage proves those events are deferred while ordinary ASCII and navigation keys continue to the existing PTY encoder.

**Verified:**
- `imeCaretStyle` unit-tested (`terminalIme.test.ts`) for the cursor-to-pixel transform math.
- Pre-composition routing is unit-tested for macOS dead keys, WebKit process keys, key-code 229, active composition, normal ASCII, and navigation input.
...
- Code-path review: `send_key`/`Msg::Key` (Rust) remains unchanged. The JS key listener now has one explicit pre-composition yield rule, covered by the routing tests above.
