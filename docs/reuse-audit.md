# Reuse Audit (REUSE-AUDIT, amended 2026-07-17)
...
| `rockmap/` | Live tooling: `build-roadmap.mjs` renders `roadmap.json` → `roadmap.html`; `roadmap.schema.json` drives editor validation. Demo/package collateral was removed in the public-repo cleanup. | `roadmap.html`, every roadmap commit |
...
| `resources/superconductor-reference/NOTES.md` | Sanitized UX-pattern notes only. The extracted competitor icon and `Info.plist` were removed on 2026-07-17 after their reference value expired. | Reinspect the installed app only when a concrete workflow needs it. |
...
| `app/src-tauri/src/harness.rs`, `web/server/routes/sessions-run.ts` | Adapted behavior, independently implemented | Previewed parallel dispatch now creates durable provider-backed child chats with explicit budgets, worktree ownership, selected-run cancellation, attributed return, file-conflict warnings, and per-child failure recovery. Anonymous child loops against the parent session were rejected. |
...
- **Third-party binary collateral is gone.** The public repository keeps only written reference notes; Keelhouse's own SVG and generated bundle assets own product identity.
