...
Connection records, process environment injection, live MCP discovery, provider recovery launch, and the OAuth protocol lifecycle are implemented. Claude's structured adapter reuses the existing Claude CLI session; its capability and auth status have been inspected without sending a prompt. Local socket tests execute metadata discovery, dynamic registration, code exchange, refresh-token preservation, and revocation. Gemini was switched from an invalid API-key selection to its official `oauth-personal` flow, but the first smoke stopped at browser consent while the Mac was locked. This does not prove third-party interoperability, the packaged browser-consent workflow, or Claude's packaged turn lifecycle. A real MCP OAuth server and real provider flows must still be validated through the packaged UI before `AI-CONNECTIONS` is complete.
