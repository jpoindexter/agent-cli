...
- Existing app-owned paths now route through the gate: focus pane, open file, open diff, open browser preview, interrupt process, restart process, terminate process, create pane, close pane, attach reference, and composer app commands (`>save`, `>find`, `>open`, `>clear`).
...
|---|---:|---|
| Focus pane, open file, open diff, find, browser preview, attach reference | Low | Navigation is reversible by selecting the previous target; diff review can be closed; attachments can be removed. |
